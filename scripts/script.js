...
const checkHpLevels = (pokemonHp, initiatingPerson, receivingPlayer) => {
    ...
    if(pokemonHp <= 0 ){
        ...
        battleText.innerText = `${receivingPlayer.name}'s ${receivingPlayer.pokemon.pokemonName.toUpperCase()} fainted!  Play again?`;

        submitButton.style.display = 'none';
        newBattle.style.display = 'inline-block';
    } else if(initiatingPerson === opponent) {
        console.log(initiatingPerson);
        setTimeout(() => battleText.innerText = `What will ${player.pokemon.pokemonName.toUpperCase()} do?`, 1000);
    }
...
const attack = (move, attackingPlayer, receivingPlayer) => {
    axios.get(`https://pokeapi.co/api/v2/move/${move}`)
        .then(res => {
            ...
            battleText.innerText = `${attackingPlayer.name}'s ${attackingPlayer.pokemon.pokemonName.toUpperCase()} used ${move} on ${receivingPlayer.pokemon.pokemonName.toUpperCase()}`
        })
        .then(() => {
            checkHpLevels(receivingPlayer.pokemon.pokemonHp, attackingPlayer, receivingPlayer);
        })
        .catch(console.log)
...
const attackCalculation = (power, recipientOfAttack) => {
    ...
    recipientOfAttack.pokemon.pokemonHp = recipientOfAttack.pokemon.pokemonHp - attackValue;
